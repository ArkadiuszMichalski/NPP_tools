...
function getPluginZip(url, plugCache, folder_ver, plugin){
	...
	correctTool.get(url, function(res){
		...
		res.on("end", function(){
			...
			if (plugin.id.toLowerCase() == hash.toLowerCase()){

					console.log("Download hash: OK");
				...
				console.log("Download hash: BAD");
				currentStep += 1;
				downloadBAD += 1;
				downloadBADinfo += "\n" + folder_ver + ": BAD hash for " + url;
...
function isFinish(){
	...
	if ((fixedSteps <= 0 && currentStep >= totalStep) || (fixedSteps > 0 && currentStep >= fixedSteps)){
